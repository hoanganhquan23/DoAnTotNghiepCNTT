...
const ChatBox = ({ onClose, messages, chatRoomId }) => {
  ...
  // Gọi API OpenRouter lấy trả lời AI
  ...
  const handleSendMessage = async () => {
    ...
    if (inputMessage.toLowerCase().includes("@ai")) {
      ...
      // Gọi API AI, lấy câu trả lời
      const prompt = inputMessage.replace(/@ai/gi, "").trim();
      const aiResponseHtml = await callOpenRouterAPI(prompt);
  ...
  // Cuộn xuống cuối mỗi khi msgList thay đổi
  useEffect(() => {
